...
| 属性名 | 类型 | 说明 |
|------|------|------|
| `_goal_shares` | tuple | **(w_home, w_hoard, w_herd)** I1 三阶段归一化权重；启用 P3 时扩为 4-tuple **(w_home, w_hoard, w_herd, w_inquire)**，由 compute_goal_direction() 写入 |
...
| 字段路径 | 类型 | 说明 |
|---|---|---|
| `config.city / district / n_residents / seed / tag` | str/int | 实验配置快照 |
| `config.home_distribution` | str | F2 加入, `'poi'` 或 `'uniform'` |
...
**统计实现**: `_pearson_r_p()` / `_spearman_rho_p()` 用纯 Python 求和公式手算 (绕过 scipy/numpy BLAS, Crowds_sim env 在 >5000 长 array 会 kernel-level crash)。p-value 用 t 分布近似 + `math.erfc` 标准正态 CDF。
