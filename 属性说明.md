...
**统计实现**: `_pearson_r_p()` / `_spearman_rho_p()` 直接调 `scipy.stats.pearsonr` / `spearmanr` (6-27 翻案: 原来怀疑的 BLAS crash 是 conda env DLL 路径问题, 通过 `.venv/run_in_crowds_env.ps1` wrapper 启动后 scipy 1.18 + numpy 2.2 在 n=6000 长 array 上完全正常)。
